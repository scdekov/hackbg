...
def create_language_folder(language):
    call("mkdir %s" % (language), shell=True)
...
def create_language_list(conn):
    ...
    pattern = "%s [%d] - %s"
    l = lambda x: pattern % (get_language_answered_state(x[3]), x[0], x[1])
...
def trigger_start(conn, command):
    ...
    query_sources = "SELECT file_name, source FROM sources WHERE lang_id = %d"
...
def main():
    conn = open_connection("compiler.db")
